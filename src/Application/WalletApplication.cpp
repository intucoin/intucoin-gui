// Copyright (c) 2017-2018, IntuCoin developers
...
const char BYTECOIN_URI_SCHEME_NAME[] = "intucoin";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("intucoin-wallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] IntuCoin wallet is already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "IntuCoin wallet is already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have intucoind with non-default RPC port, you should terminate it and relaunch IntuCoin wallet\n"
      "or\n"
      "Set the Local deamon required port in IntuCoin wallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
